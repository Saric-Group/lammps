...
#include "pair_cosine_squared_surface.h"
...
PairCosineSquaredSurface::PairCosineSquaredSurface(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
PairCosineSquaredSurface::~PairCosineSquaredSurface()
{
  if (allocated) {
...
void PairCosineSquaredSurface::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCosineSquaredSurface::settings(int narg, char **arg)
{
  if (narg != 1) {
...
void PairCosineSquaredSurface::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 6)
  ...
  if (cut_one < sigma_one) {
    ...
    if (wca_one == 0) {
      ...
      error->warning(FLERR, "cosine/squared/surface set to WCA only (cutoff = sigma)" + utils::errorurl(21));
    }
  }
...
double PairCosineSquaredSurface::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
    error->all(FLERR, "Mixing not supported in pair_style cosine/squared/surface");
...
void PairCosineSquaredSurface::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "mix") == 0) {
      error->all(FLERR, "pair_modify mix not supported for pair_style cosine/squared/surface");
    } else if (strcmp(arg[iarg], "shift") == 0) {
      error->warning(FLERR, "pair_modify shift has no effect on pair_style cosine/squared/surface");
      offset_flag = 0;
    } else if (strcmp(arg[iarg], "tail") == 0) {
      error->warning(FLERR, "pair_modify tail has no effect on pair_style cosine/squared/surface");
      tail_flag = 0;
    }
    iarg++;
  }
...
void PairCosineSquaredSurface::write_restart(FILE *fp)
{
...
void PairCosineSquaredSurface::read_restart(FILE *fp)
{
...
void PairCosineSquaredSurface::write_restart_settings(FILE *fp)
{
...
void PairCosineSquaredSurface::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairCosineSquaredSurface::write_data(FILE *fp)
{
...
void PairCosineSquaredSurface::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairCosineSquaredSurface::compute(int eflag, int vflag)
{
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        */
        ...
        if (itype == 5) {
            ...
            f[i][0] -= align * fpair * normx;
            f[i][1] -= align * fpair * normy;
            f[i][2] -= align * fpair * normz;
...
double PairCosineSquaredSurface::single(int /* i */, int /* j */, int itype, int jtype, double rsq,
